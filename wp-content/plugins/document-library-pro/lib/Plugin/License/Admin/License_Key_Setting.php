...
namespace Barn2\DLP_Lib\Plugin\License\Admin;

use Barn2\DLP_Lib\Admin\Settings_API_Helper;
use Barn2\DLP_Lib\Plugin\License\License;
...
class License_Key_Setting implements Registerable, License_Setting {
		...
		if ( $this->is_license_action( self::ACTIVATE_KEY ) ) {
			...
			if ( isset( $license_setting['license'] ) ) {
				...
				$this->add_settings_message(
					__( 'License key successfully activated.', 'document-library-pro' ),
					__( 'There was an error activating your license key.', 'document-library-pro' ),
					$activated
				);
			}
			...
			$this->add_settings_message(
				__( 'License key successfully deactivated.', 'document-library-pro' ),
				__( 'There was an error deactivating your license key, please try again.', 'document-library-pro' ),
				$deactivated
			);
			...
			$this->add_settings_message(
				__( 'The license key looks good!', 'document-library-pro' ),
				__( 'There\'s a problem with your license key.', 'document-library-pro' ),
	...
	public function get_license_key_setting() {
		$setting = [
			'title' => __( 'License key', 'document-library-pro' ),
			'type'  => 'text',
	...
	private function get_license_description() {
		$buttons = [
			'check'      => $this->license_action_button( self::CHECK_KEY, __( 'Check', 'document-library-pro' ) ),
			'activate'   => $this->license_action_button( self::ACTIVATE_KEY, __( 'Activate', 'document-library-pro' ) ),
			'deactivate' => $this->license_action_button( self::DEACTIVATE_KEY, __( 'Deactivate', 'document-library-pro' ) )
		];
