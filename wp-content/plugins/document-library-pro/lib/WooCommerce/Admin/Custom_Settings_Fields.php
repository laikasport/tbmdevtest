<?php
namespace Barn2\DLP_Lib\WooCommerce\Admin;
...
class Custom_Settings_Fields implements Registerable {
    ...
    public function register() {
        foreach ( $this->fields_required as $field ) {
            if ( ! has_action( "woocommerce_admin_field_{$field}" ) && method_exists( Custom_Settings_Fields::class, "{$field}_field" ) ) {
                add_action( "woocommerce_admin_field_{$field}", [ Custom_Settings_Fields::class, "{$field}_field" ] );
            }

            if ( $field === 'checkbox_tooltip' ) {
                add_filter( 'woocommerce_admin_settings_sanitize_option', [ Custom_Settings_Fields::class, 'sanitize_checkbox_tooltip_field' ], 10, 3 );
            }
        }
    }
        ...
        <tr valign="top">
            ...
            <td class="forminp forminp-<?php echo sanitize_title( $value['type'] ) ?> color-size-field">&lrm;
                ...
                <input
                    ...
                    placeholder="<?php _e( 'Color', 'document-library-pro' ); ?>"
                    />&lrm;
                ...
                <input
                    ...
                    placeholder="<?php _e( 'Size', 'document-library-pro' ); ?>"
