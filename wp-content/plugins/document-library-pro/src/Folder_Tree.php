...
class Folder_Tree {
	...
	private function get_attached_categories() {
		...
		if ( ! empty( $this->include_terms ) ) {
			...
			$categories = array_filter(
				...
				function( \WP_Term $item ) use ( $include_terms, $field ) {
					return in_array( $item->{$field}, $include_terms, true );
				}
			);
		}
		...
		if ( ! empty( $this->exclude_terms ) ) {
			...
			$categories = array_filter(
				...
				function( \WP_Term $item ) use ( $exclude_terms, $field ) {
					return ! in_array( $item->{$field}, $exclude_terms, true );
				}
			);
		}
		...
		if ( ! empty( $this->include_categories ) ) {
			...
			$categories = array_filter(
				...
				function( \WP_Term $item ) use ( $field ) {
					return in_array( $item->{$field}, $this->include_categories, true );
				}
			);
		}
		...
		if ( ! empty( $this->exclude_categories ) ) {
			...
			$categories = array_filter(
				$categories,
				function( \WP_Term $item ) use ( $field ) {
					return ! in_array( $item->{$field}, $this->exclude_categories, true );
				}
			);
		}
	...
	private function maybe_output_search_box( $context = 'top' ) {
		...
		if ( in_array( $search_position, [ $context, 'both' ], true ) ) {
			...
			$search_html = sprintf(
				'<div class="dlp-folders-search"><label>%1$s<input type="search" value="%2$s"></label><div class="dlp-folders-reset"><a class="reset" href="#">%3$s %4$s</a></div></div>',
