...
"position","textAlign","display","height","overflow"]}function div(args){return tag("div",args)}function span(args){return tag("span",args)}function emptyNode(){return document.createTextNode("")}function a(args){var output=tag("a",args);if("undefined"===typeof args.href)args.href="#";if("undefined"!==typeof args.target)output.target=args.target;output.setAttribute("href",args.href);return output}function button(args){if("undefined"===typeof args.class)args.class=[];else if(!Array.isArray(args.class))args.class=
[args.class];args.class.push("button");if(-1!==args.class.indexOf("frm-button-secondary"))args.class.push("button-secondary");else if(-1===args.class.indexOf("frm-views-modal-cancel-button"))args.class.push("button-primary","frm-button-primary");return a(args)}function select(options,args){var output=tag("select",{children:options}),has=makeHas(args);if(has("id")){output.id=args.id;if(has("onchange")){output.id=args.id;jQuery(document).off("change","#"+args.id);jQuery(document).on("change","#"+args.id,
args.onchange)}}return output}function option(value,textContent,isSelected){var output=document.createElement("option");output.setAttribute("value",value);output.textContent=textContent;if(isSelected)output.setAttribute("selected","selected");return output}function pullOptionsFrom(templateId,selectedValue){var optionElements,optionIndex,currentOption,currentOptionValue,isSelected,template=document.getElementById(templateId),options=[];if(null===template)return options;if("number"===typeof selectedValue)selectedValue=
selectedValue.toString();optionElements=template.querySelectorAll("option");for(optionIndex=0;optionIndex<optionElements.length;++optionIndex){currentOption=optionElements[optionIndex];currentOptionValue=currentOption.getAttribute("value");if(""!==currentOptionValue){isSelected=currentOptionValue===selectedValue;options.push(option(currentOptionValue,currentOption.textContent,isSelected))}}return options}function radios(options,name){var keys=Object.keys(options);return span({children:map(keys,function(key){return input({id:"radio_"+
name+"_"+key,name:name,type:"radio",label:options[key],value:key})}),class:"frm-dom-radios"})}function input(args){var label,setInputAttribute,has=makeHas(args),output=tag("input",args);if(!has("type"))args.type="text";setInputAttribute=function(attr){if(has(attr))output[attr]=args[attr]};each(getSupportedInputAttrs(),setInputAttribute);if(has("label")){label=tag("label",{text:args.label});if(has("id"))label.setAttribute("for",args.id);output=div({children:[div({child:label}),output]})}if(has("onchange"))if(has("id"))jQuery(document).on("change",
"#"+args.id,args.onchange);else console.warn("formidable-views: please provide an id when setting an onchange");return output}function getSupportedInputAttrs(){return["value","placeholder","type","name"]}function tabs(items,args){var passArgs,addTab,outputArgs,has=makeHas(args),state={},children=[];if(has("active"))state.activeTab=args.active;else if("undefined"!==items[0].pageKey)state.activeTab=items[0].pageKey;else state.activeTab=items[0].text;passArgs=function(tabArgs,key,mapKey){if(!has(key))return;
if("undefined"!==typeof mapKey)tabArgs[mapKey]=args[key];else tabArgs[key]=args[key]};addTab=function(tabArgs,index){var newTab;passArgs(tabArgs,"tabClass","class");passArgs(tabArgs,"pageWrapper");passArgs(tabArgs,"stateHandler");newTab=tab(tabArgs,index,state);if(newTab.classList.contains("frm-active-tab"))state.activeTabElement=newTab;children.push(newTab)};each(items,addTab);outputArgs={children:children};if(has("wrapperClass"))outputArgs.class=args.wrapperClass;return div(outputArgs)}function tab(tabArgs,
index,state){var element,onclick,isActive,setPage,has=makeHas(tabArgs);if(!has("pageKey"))tabArgs.pageKey=tabArgs.text;isActive=function(index){return tabArgs.pageKey===state.activeTab};setPage=function(){if(has("page")&&has("pageWrapper")){if(has("stateHandler")&&has("pageKey"))tabArgs.stateHandler(tabArgs.pageKey);redraw(tabArgs.pageWrapper,tabArgs.page())}};if(!Array.isArray(tabArgs.class))tabArgs.class=[tabArgs.class];if(isActive(index))tabArgs.class.push("frm-active-tab");if("undefined"!==typeof tabArgs.onclick)onclick=
tabArgs.onclick;else onclick=function(){};tabArgs.onclick=function(){state.activeTabElement.classList.remove("frm-active-tab");element.classList.add("frm-active-tab");state.activeTabElement=element;setPage();onclick()};element=div(tabArgs);if(isActive())setPage();return element}function collapsible(header,child,args){var content,section,classes=["accordion-section"],has=makeHas(args),open=has("open")&&args.open;header.classList.add("accordion-section-title");header.onclick=function(){open=!open;content.setAttribute("aria-expanded",
open);jQuery(section).toggleClass("open",open);if(section.classList.contains("open"))tryToFocusOnFirstAvailableField(section)};content=div({child:child,class:"accordion-section-content"});content.setAttribute("aria-expanded",open);if(open)classes.push("open");section=div({children:[header,content],class:classes});return section}function tryToFocusOnFirstAvailableField(section){var field=section.querySelector("input");if(null===field)field=section.querySelector("textarea");if(null!==field)field.focus()}
function redraw(element,child){element.innerHTML="";element.appendChild(child)}function maybeCreateModal(args){var metaboxHolder,postbox,top,title,modal,top,content,footer,modalHelper,has=makeHas(args);if("undefined"===typeof args)args={};if(has("id"))modal=document.getElementById(args.id);if(null===modal||"undefined"===typeof modal){modal=div({class:"frm-modal"});if(has("tabindex"))modal.tabIndex=args.tabindex;if(has("id"))modal.id=args.id;postbox=div({class:"postbox"});metaboxHolder=div({class:"metabox-holder",
child:postbox});modal.appendChild(metaboxHolder);document.body.appendChild(modal)}postbox=modal.querySelector(".postbox");modalHelper=getModalHelper(modal,postbox);if(has("title")){title=modal.querySelector(".frm-modal-title");if(null===title){title=div({child:document.createTextNode(args.title),class:"frm-modal-title"});top=modal.querySelector(".frm_modal_top");if(null===top){top=div({class:"frm_modal_top",children:[title,div({child:a({text:__("Cancel","formidable-views"),class:"dismiss"})})]});
postbox.appendChild(top)}}else title.innerHTML=div({child:document.createTextNode(args.title)}).innerHTML}if(has("content"))modalHelper(args.content,"frm_modal_content");if(has("footer"))modalHelper(args.footer,"frm_modal_footer");makeModalIntoADialogAndOpen(modal);return modal}function getModalHelper(modal,appendTo){return function(child,uniqueClassName){var element=modal.querySelector("."+uniqueClassName);if(null===element){element=div({child:child,class:uniqueClassName});appendTo.appendChild(element)}else redraw(element,
child)}}function makeModalIntoADialogAndOpen(modal){var $modal=jQuery(modal);if(!$modal.hasClass("frm-dialog"))$modal.dialog({dialogClass:"frm-dialog",modal:true,autoOpen:false,closeOnEscape:true,width:"550px",resizable:false,draggable:false,open:function(){jQuery(".ui-dialog-titlebar").addClass("frm_hidden").removeClass("ui-helper-clearfix");jQuery("#wpwrap").addClass("frm_overlay");jQuery(".frm-dialog").removeClass("ui-widget ui-widget-content ui-corner-all");$modal.removeClass("ui-dialog-content ui-widget-content");
jQuery(".ui-widget-overlay, a.dismiss").bind("click",function(event){event.preventDefault();$modal.dialog("close")})},close:function(){jQuery("#wpwrap").removeClass("frm_overlay");jQuery(".spinner").css("visibility","hidden")}});scrollToTop();$modal.dialog("open");return $modal}function scrollToTop(){if("scrollRestoration"in history)history.scrollRestoration="manual";window.scrollTo(0,0)}function recenterModal(modalId){jQuery(document.getElementById(modalId)).dialog("option","position",{my:"center",
at:"center",of:window})}function each(items,callback){var index,length=items.length;for(index=0;index<length;index++)callback(items[index],index)}function map(items,callback){var output=[];each(items,function(item,index){output.push(callback(item,index))});return output}function makeHas(args){if("undefined"===typeof args)return function(){return false};return function(key){return"undefined"!==typeof args[key]}}function post(data,success,error){var handleSuccessCallback=function(responseData){if("function"===
typeof success)success(responseData)};jQuery.ajax({type:"POST",url:ajaxurl,data:data,success:function(response){if("undefined"!==typeof response.success&&response.success){if("undefined"===typeof response.data){handleSuccessCallback({});return}handleSuccessCallback(response.data);if("undefined"!==typeof response.data.redirect)window.location=response.data.redirect}else if("function"===typeof error)error(response)},error:error})}function success(content){var container,notice;container=document.getElementById("wpbody");
notice=div({class:["notice","notice-info","frm-review-notice","frm_updated_message"],child:div({class:"frm-satisfied",child:content})});notice.style.borderRadius="4px";notice.style.right="10px";container.appendChild(notice);setTimeout(function(){jQuery(notice).fadeOut(function(){notice.remove()})},2E3)}function getTableSelectAllAndSearchOptions(){return div({class:"frm_grid_container",margin:"0 0 20px",children:[div({class:"frm_fourth",padding:"10px 0 0 ",child:a({id:"frm_table_columns_select_all",
onclickPreventDefault:function(){toggleAllColumnOptions(true)},text:__("Select all","formidable-views")})}),div({class:"frm_fourth",padding:"10px 0 0 ",child:a({id:"frm_table_columns_deselect_all",onclickPreventDefault:function(){toggleAllColumnOptions(false)},text:__("Deselect all","formidable-views")})}),div({class:"frm_half",child:getTableColumnOptionSearch()})]})}function toggleAllColumnOptions(checked){each(document.querySelectorAll(".frm-new-table-view-option input"),function(option){option.checked=
...
onchange:function(){updateViewData("name",this.value);document.getElementById("frm_bs_dropdown").querySelector("h1 span").textContent=this.value},label:__("View Name","formidable-views"),value:frmViewsEditorViewData.name}),getApplicationDropdown(),getShortcodeExample(),getPhpEchoExample(),separator(),getSettingsForActiveViewType(),getDisplaySettings(),getMetaboxes()];noEntriesTextareaWrapper=collapsible(div({text:__("No Entries Message","formidable-views")}),getNoEntriesTextarea());noEntriesTextareaWrapper.id=
"frm_no_entries_textarea_collapsible";children.push(noEntriesTextareaWrapper);return children}function updateViewData(key,value){frmViewsEditorViewData[key]=value;preventUnsavedChanges()}function getViewKeyInput(){var output=wrapFormInput({id:"frm_views_view_key",onchange:function(){state.viewKey=this.value;preventUnsavedChanges()},label:__("View Key","formidable-views"),value:frmViewsEditorViewData.viewKey});output.style.margin="10px 0 0";return output}function getDetailSlugPlaceholder(){var placeholder=
dataProcessor.placeholder("detailSlug",maybeGetDetailSlugInput);placeholder.id="frm_views_detail_slug_wrapper";return placeholder}function maybeGetDetailSlugInput(detailSlug){var wrappedInput;if(!viewIsDynamic())return emptyNode();wrappedInput=wrapFormInput({id:"frm_views_detail_slug",class:"frm_half",onchange:function(){updateDataProcessor("detailSlug",this.value);refreshLayoutBuilderArea()},label:__("Detail Page Slug","formidable-views"),value:detailSlug});if(isSingleEntryType()||isCalendarType())wrappedInput.style.marginTop=
0;appendTooltipTriggerAferInputLabel(wrappedInput,getDetailSlugInputTooltipText());wrappedInput.firstChild.classList.add("frm_grid_container");jQuery(wrappedInput.querySelector("#frm_views_detail_slug")).after(getParameterValueDropdown());return wrappedInput}function getDetailSlugInputTooltipText(){var text=__("Example: If parameter name is 'contact', the url would be like %1$s/selected-page?contact=2. If this entry is linked to a post, the post permalink will be used instead.","formidable-views");
return text.replace("%1$s",frmViewsEditorInfo.siteUrl)}function getParameterValueDropdown(){var dropdown=select([option("id","ID","id"===dataProcessor.data.parameterValue),option("display_key","Key","display_key"===dataProcessor.data.parameterValue)]);dropdown.id="frm_views_parameter_value";dropdown.classList.add("frm_half");jQuery(document).off("change","#"+dropdown.id);jQuery(document).on("change","#"+dropdown.id,function(){updateDataProcessor("parameterValue",this.value);refreshLayoutBuilderArea()});
...
"none":"grid"}function getFormSelectWrapper(){return div({margin:"10px 0 0",children:[getFormSelectLabel(),getFormSelect()]})}function getLimitPlaceholder(){var limitPlaceholder=dataProcessor.placeholder("limit",getViewLimitInput);limitPlaceholder.classList.add("frm_half");return limitPlaceholder}function getTooltipTrigger(text,placement){var output=span({class:["frm_help","frm_icon_font","frm_tooltip_icon"]});output.setAttribute("title",text);output.style.position="relative";output.style.top="3px";
output.style.left="4px";if("undefined"!==typeof placement)output.setAttribute("data-placement",placement);return output}function getPageSizePlaceholder(){var pageSizePlaceholder=dataProcessor.placeholder("pageSize",getPageSizeInput);pageSizePlaceholder.classList.add("frm_half");return pageSizePlaceholder}function getCalendarSettings(){return collapsible(div({text:__("Calendar Settings","formidable-views")}),dataProcessor.placeholder("fieldOptions",function(){return div({children:[div({children:[tag("label",
{text:__("Event Date","formidable-views")}),select(getEventDateOptions(dataProcessor.data.dateFieldId),{id:"frm_date_field_id",onchange:function(){updateDataProcessor("dateFieldId",this.value)}})]}),div({margin:"10px 0 0",children:[tag("label",{text:__("End Date or Day Count","formidable-views")}),select(getEndDateOrDayCountOptions(dataProcessor.data.edateFieldId),{id:"frm_edate_field_id",onchange:function(){updateDataProcessor("edateFieldId",this.value)}})]}),div({class:"frm_grid_container",children:[getRepeatField(),
div({margin:"10px 0 0",class:"frm_half",children:[getEndRepeatLabel(),getEndRepeatSelect()]})]})]})}),{open:true})}function getRepeatField(){var output=div({margin:"10px 0 0",class:"frm_half",children:[tag("label",{text:__("Repeat","formidable-views")}),select(getEventRepeatOptions(dataProcessor.data.repeatEventFieldId),{id:"frm_repeat_event_field_id",onchange:function(){updateDataProcessor("repeatEventFieldId",this.value)}})]});appendTooltipTriggerAferInputLabel(output,getRepeatFieldTooltipText());
return output}function getRepeatFieldTooltipText(){return __("Select a form field that contains values like 1 week, 2 weeks, 1 year, etc. This will set the repeat period for each event.","formidable-views")}function getEndRepeatLabel(){var output=tag("label",{text:__("End Repeat","formidable-views")});if(""===dataProcessor.data.repeatEventFieldId)output.classList.add("frm-disabled-label");return output}function getEndRepeatSelect(){var output=select(getEventEndRepeatOptions(dataProcessor.data.repeatEdateFieldId),
...
if(isCalendarType())return getCalendarSettings();if(isTableType())return getTableSettings();return emptyNode()}function getGridStyleSettings(){return collapsible(div({text:__("Grid Style Settings","formidable-views")}),dataProcessor.placeholder("gridColumnCount",function(){setTimeout(initializeColorPicker,0);return div({children:[getCellTypographySettings(),getCellBackgroundSettings(),getCellBorderSettings(),getCellBorderRadiusSettings(),getCellPaddingSettings(),getCellStyleSettings(__("Number of grid columns",
"formidable-views"),div({child:getNumberOfGridColumnsInput()})),getCellStyleSettings(__("Row gap","formidable-views"),div({child:getGridRowGapInput()})),getCellStyleSettings(__("Column gap","formidable-views"),div({child:getGridColumnGapInput()})),getGridResponsivenessWrapper(),getCustomClassesSettings()]})}),{open:true})}function getCustomClassesSettings(){return getCellStyleSettings(tag("label",{text:__("Custom classes","formidable-views"),for:"frm_custom_classes_input"}),div({class:"frm-full-width-settings-input",
child:getCustomClassesInput()}))}function getTableSettings(){return collapsible(div({text:__("Table Settings","formidable-views")}),div({children:[tag("label",{text:__("Row Style","formidable-views")}),dataProcessor.placeholder("tableRowStyle",getTableRowStyleDropdown),getTableResponsivenessWrapper(),dataProcessor.placeholder("customClasses",getCustomClassesSettings)]}),{open:true})}function getTableRowStyleDropdown(tableRowStyle){return select([option("",__("Match theme settings","formidable-views"),
""===tableRowStyle),option("frm-alt-table",__("Zebra stripes","formidable-views"),"frm-alt-table"===tableRowStyle),option("frm-grid",__("Grid","formidable-views"),"frm-grid"===tableRowStyle)],{id:"frm_table_row_style",onchange:function(){var table=document.getElementById("frm_table_editor_table");dataProcessor.data.tableRowStyle=this.value;jQuery(table).removeClass("frm-alt-table frm-grid");if(""!==this.value)table.classList.add(this.value)}})}function getTableResponsivenessWrapper(){return dataProcessor.placeholder("tableResponsive",
function(){var checkbox,label,wrapper;checkbox=getTableResponsivenessInput();label=tag("label",{text:__("Make table responsive","formidable-views"),fontSize:"14px",for:checkbox.id});wrapper=div({margin:"5px 0 0",children:[checkbox,label]});appendTooltipTriggerAferInputLabel(wrapper,__("If this is on, the each column will display as a full row when the browser window is smaller than 760px width.","formidable-views"),"right");return wrapper})}function getTableResponsivenessInput(){var output=input({id:"frm_table_responsive_"+
autoId(),onchange:handleTableResponsivenessChange,type:"checkbox"});output.checked=!!dataProcessor.data.tableResponsive;return output}function handleTableResponsivenessChange(){dataProcessor.data.tableResponsive=this.checked?1:0;preventUnsavedChanges()}function getGridResponsivenessWrapper(){var checkbox,label,wrapper;checkbox=getGridResponsivenessInput();label=tag("label",{text:__("Make grid responsive","formidable-views"),fontSize:"14px",for:checkbox.id});wrapper=div({children:[checkbox,label]});
appendTooltipTriggerAferInputLabel(wrapper,__("If this is on, the number of grid columns will go to one when the browser window is smaller than 750px width.","formidable-views"),"right");return wrapper}function getFieldOptions(types,selectedValue){var results=[];if(-1!==types.indexOf("created_at"))results.push(option("created_at",__("Entry creation date","formidable-views"),"created_at"===selectedValue));if(-1!==types.indexOf("updated_at"))results.push(option("updated_at",__("Entry update date","formidable-views"),
"updated_at"===selectedValue));each(dataProcessor.data.fieldOptions,function(fieldOption){each(types,function(type){if(type===fieldOption.type)results.push(option(fieldOption.value,fieldOption.label,selectedValue===fieldOption.value))})});return results}function getMetaboxes(){var wrapper=document.getElementById("frm_view_editor_metabox_wrapper");if(null===wrapper)return emptyNode();return collapsible(div({text:__("Metaboxes","formidable-views")}),wrapper)}function getStyleEditorLeftPanelChildren(){return[getCellSettingsHeader(),
separator(),getCellTypographySettings(),getCellBackgroundSettings(),getCellBorderSettings(),getCellBorderRadiusSettings(),getCellPaddingSettings()]}function getCellSettingsHeader(){var settings=getCellStyleSettings(__("Cell Settings","formidable-views"),div({child:getCellSettingsDoneButton()}));settings.firstChild.style.fontWeight="600";return settings}function getCellSettingsDoneButton(){return button({id:"frm_done_cell_style_settings",onclickPreventDefault:function(){toggleOffCellStyleSettings();
...
children:[checkbox,label]})}function getDisablePreviewCheckbox(disablePreview){const id="frm_views_disable_preview";const labelText=__("Show code for this view","formidable-views");const checked=disablePreview;const {checkbox,label}=getSettingsCheckboxElements(id,labelText,checked);if(disablePreview)toggleCodeViewActiveClass(true);checkbox.disabled=isLegacyTableType();checkbox.addEventListener("change",()=>{updateDataProcessor("disablePreview",checkbox.checked?1:0);toggleCodeViewActiveClass(checkbox.checked);
refreshActivePreviews()});return div({margin:"10px 0 0",children:[checkbox,label,getTooltipTrigger(getShowCodeForThisViewTooltipText())]})}function getSettingsCheckboxElements(id,labelText,checked){const checkboxArgs={id};const checkbox=tag("input",checkboxArgs);checkbox.setAttribute("type","checkbox");const label=tag("label",{text:labelText,for:checkbox.id});if(checked)checkbox.checked=true;return{checkbox,label}}function toggleCodeViewActiveClass(on){jQuery(document.getElementById("frm_view_editor_preview_area")).toggleClass("frm-code-view-active",
...
"multiple");options.forEach(option=>dropdown.appendChild(option));container.appendChild(label);container.appendChild(dropdown);container.className="frm-fields";container.style.margin="15px 0 10px";const $dropdown=jQuery(dropdown);$dropdown.hide().each(frmDom.bootstrap.multiselect.init);$dropdown.on("frm-multiselect-changed",adustValuesForApplicationDropdown)});return container}function adustValuesForApplicationDropdown(_,option){if(""!==option.getAttribute("value"))unselectBlankOptionIfSelected(jQuery(this))}
...
typeof frmViewsTable)getTableSelectAllAndSearchOptions=frmViewsTable.getTableSelectAllAndSearchOptions}function prepareViewDropdown(){if("undefined"===typeof frmDom||"object"!==typeof frmDom.ajax)return;const {doJsonPost}=frmDom.ajax;const formData=new FormData;formData.append("viewId",frmViewsEditorViewData.id);doJsonPost("view_dropdown_options",formData).then(maybeReplaceViewHeaderWithDropdown)}function maybeReplaceViewHeaderWithDropdown(data){if(!data||!data.options)return;const topBar=document.getElementById("frm_top_bar");
const h1=topBar.querySelector("h1");const anchor=a({className:"frm-dropdown-toggle"});topBar.classList.add("frm-with-dropdown");anchor.setAttribute("data-toggle","dropdown");h1.appendChild(document.createTextNode(" "));h1.appendChild(frmDom.svg({href:"#frm_arrowdown4_icon"}));h1.parentNode.replaceChild(anchor,h1);anchor.appendChild(h1);const dropdownMenu=frmDom.div({className:"frm-dropdown-menu frm_code_list frm-full-hover frm-inline-modal",children:data.options.map(getViewDropdownItem)});const searchWrapper=
frmDom.div({className:"dropdown-item frm-with-search",child:getViewDropdownSearch()});jQuery(anchor.parentNode).on("show.bs.dropdown",autoFocusSearchOnDropdownShow);dropdownMenu.insertBefore(searchWrapper,dropdownMenu.firstChild);anchor.parentNode.insertBefore(dropdownMenu,anchor.nextElementSibling);function autoFocusSearchOnDropdownShow(){setTimeout(()=>searchWrapper.querySelector("input").focus(),0)}}function getViewDropdownItem(view){if("string"===typeof view.categoryHeader)return frmDom.div({className:"dropdown-header frm-dropdown-view",
text:view.categoryHeader});const item=frmDom.div({className:"dropdown-item frm-dropdown-view",child:frmDom.a({children:[frmDom.span({className:"frm-sub-label",text:"(ID "+view.id+")"}),document.createTextNode(view.name)],href:view.editUrl.replace("&amp;","&")})});const searchText=(view.name+" "+view.id+" "+view.category).toLowerCase();item.setAttribute("frm-search-text",searchText);return item}function getViewDropdownSearch(){const {newSearchInput}=frmDom.search;const id="frm-view-dropdown-search-input";
const placeholder=__("Search views","formidable-views");const targetClassName="frm-dropdown-view";return newSearchInput(id,placeholder,targetClassName)}setupExternalFunctionReferences();setupDataProcessor();setupBoxProcessor();prepareViewDropdown();jQuery(document).ready(initialize)})();
