...
*/
		...
		<tbody class="emp-static-map-options">
			...
			$static_link_options = array(
				'none' => __('No Action', 'em-pro'),
				'link' => __('Link to maps.google.com and center on coordinates', 'em-pro'),
				'link-place' => __('Link to maps.google.com based on location name and address', 'em-pro'),
				'link-address' => __('Link to maps.google.com based on only address', 'em-pro'),
				'map' => __('Load dynamic map', 'em-pro'),
				'embed' => __('Load embedded map', 'em-pro'),
			);
			//dbem_gmap_static_link
			em_options_select( __('Map click action'), 'dbem_gmap_static_link', $static_link_options, esc_html__('Choose what happens when a visitor clicks on a static map.', 'em-pro'));
			em_options_input_text( __('Static hover text'), 'dbem_gmap_static_link_hover', esc_html__('If your static maps have a click action, hovering over a static map will show this text to the visitor.', 'em-pro'), __('Click for interactive map', 'em-pro'));
			...
			em_options_radio_binary(__('Enable static map caching?', 'em-pro'), 'dbem_gmap_static_cache', sprintf(esc_html__('Store a temporary copy of each map image on your own server. We strongly recommend you read our %s page about this so you ensure you are withing Google Terms of Service', 'em-pro'),$docs_link), '', '#dbem_gmap_static_cache_expiry_row');
			em_options_input_text(__('Static map cache expiry days', 'em-pro'), 'dbem_gmap_static_cache_expiry', esc_html__('Enter the amount of days a cached map image should remain on your server, we recommend a maximum of 30 days so it adheres to Google\'s Terms of Service', 'em-pro'), 28);
			?>
			<tr>
				<td colspan="2">
					<?php echo sprintf(__('Static cached maps require a server API and secret key, please see our %s page for instructions on obtaining one.', 'em-pro'), '<a href="https://wp-events-plugin.com/documentation/google-maps/static-maps/?utm_source=plugin&utm_medium=pro-settings&utm_campaign=gmaps-static-keys#api-keys">'.esc_html__('documentation','events-manager').'</a>'); ?>
				</td>
			</tr>
			<?php
		...
		<script type="text/javascript">
			jQuery(document).ready( function($){
				$('select[name="dbem_gmap_type"]').on('change', function(){
					if( $(this).val() == 'static' ){
