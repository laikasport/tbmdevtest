...
class EM_Google_Static_Maps {
	public static function init(){
		...
		if( get_option('dbem_gmap_static_cache') && get_option('dbem_gmap_static_cache_expiry') ) {
			//set up cron for addint to email queue
			if( !wp_next_scheduled('emp_cron_gmap_static_cleanup') ){
				$cleanup_time = get_option('emp_cron_gmap_static_cleanup_time');
				if( !$cleanup_time ){
					update_option('emp_cron_gmap_static_cleanup_time','02:00', false);
					$cleanup_time = '02:00';
				}
				$todays_time_to_run = strtotime(date('Y-m-d', current_time('timestamp')).' '.  $cleanup_time, current_time('timestamp'));
